...
- Dashboard layout and service tiles are configured via the Homarr web UI and persisted in the `homarr-data` named volume, backed by `/mnt/docker/stacks/dashboard` on the host
...
- All data (config, database, icons) is stored in the `homarr-data` named volume, backed by `/mnt/docker/stacks/dashboard` on the host, and persists across container restarts and updates.
